...
def load_pop_data(geojson_path, embed_dim=1024):
    """
    加载人口预测数据
    ...
    print(f"  - 有效样本: {len(X)}")
    print(f"  - 人口范围: [{y.min():.2f}, {y.max():.2f}]")
    print(f"  - 人口均值: {y.mean():.2f}")
    print(f"  - 人口中位数: {np.median(y):.2f}")

    return X, y, gdf


def train_pop_model(X_train, y_train, X_test, y_test, city_name, use_log=True):
    """
    训练人口预测模型
...
def main():
    ...
    for city in cities:
        ...
        try:
            ...
            X, y, gdf = load_pop_data(geojson_path, embed_dim=1024)
    ...
    if len(all_results) > 0:
        ...
        # 保存结果
        results_csv = metrics_dir / 'pop_prediction_satmae_results.csv'
